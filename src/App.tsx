...
function App() {
  ...
  const SetCamera = async () => {
    ...
    contextRef.current.scale(-1, 1); // Mirror cam
  ...
  async function trackFace() {
    if (AImodel && contextRef.current && glasses) {
      ...
      //TODO: get the size of the bounding box as user moves closer and farther and then scale the glasses model according to that to make it bigger or smaller
      faces.forEach(async (face) => {
        ...
        const glassesScreenPosition = new THREE.Vector3(
          -(face.annotations.midwayBetweenEyes[0][0] / canvas.width) * 2 + 1, // X coordinate in screen space [-1, 1]
          -(face.annotations.midwayBetweenEyes[0][1] / canvas.height) * 2 + 1, // Y coordinate in screen space [-1, 1]
          1 // Z coordinate in screen space (no change)
        );

        // Project the screen space coordinates to camera space
        ...
        var distance =
          ...
          glassesScreenPosition.z; // Set the glasses position to the converted camera space position
        glasses.position
        ...
        console.log("glasses", glasses.position);
